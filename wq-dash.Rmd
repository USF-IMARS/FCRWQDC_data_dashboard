...
# === load data
epcdata <- load_wq_cleaned_df(here::here("data/dashboardDataSEACAR.csv")) %>%
  filter(is.na(`Sample.Depth`) | `Sample.Depth` < 1)  # Show only surface samples
...
station_data <- reactive({
  ...
  if(length(station_parts) >= 2) {
    ...
    # Fallback to filtered main data if parsing fails
    epcdata %>% filter(epchc_station == selsit)
  }
})
...
# populated by unique values in epcdata$`Parameter` column
...
pickerInput(
  ...
  choices = sort(unique(epcdata$`Parameter`)),
  selected = sort(unique(epcdata$`Parameter`)),
  multiple = TRUE,
  options = list(
    `actions-box` = TRUE,
...
# filter epcdata for selected analytes
# update map

# master reactive data object - single source of truth for filtered data
master_filtered_data <- reactive({
  req(input$param_select, input$date_range)
  if(is.null(input$param_select) || length(input$param_select) == 0){
    return(NULL)
  }
  
  epcdata %>% 
    filter(`Parameter` %in% input$param_select) %>%
...
filtered_stations <- reactive({
  ...
  master_filtered_data() %>%
    st_drop_geometry() %>%
    distinct(epchc_station, Latitude, Longitude) %>%
...
provider_counts <- reactive({
  ...
  paste(counts$bay_segment, ":", counts$n, collapse = "\n\n")
})

# observer to update map when parameter selection or date range changes
observe({
  param_select <- input$param_select
  date_range <- input$date_range
  print(paste("param_select:", length(param_select), "items, date range:", date_range[1], "to", date_range[2]))
...
output$provider_summary <- renderPlotly({
  ...
  p <- ggplot(counts, aes(x = reorder(bay_segment, n), y = n, fill = bay_segment)) +
    geom_col() +
    coord_flip() +
    labs(
